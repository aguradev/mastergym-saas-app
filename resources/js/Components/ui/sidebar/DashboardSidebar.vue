...
import { onMounted, onUnmounted, toRefs, ref, inject, computed } from "vue";
...
const page = usePage();
const titleApp = computed(() => page.props.titlePage);
...
<template>
    <Transition name="sidebar-slide">
        <aside
            ...
            v-if="sidebarRef"
        >
            <div class="p-12 text-2xl font-bold text-center text-nowrap">
                {{ titleApp || "Master Gym" }}
            </div>

            <nav class="flex flex-col flex-1 gap-y-8">
                <Link
                    v-for="(menu, j) in menuNav[1]?.items"
                    :key="j"
                    :href="menu.link"
