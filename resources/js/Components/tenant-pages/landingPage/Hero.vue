<script setup>
    ...
    let imgUrl = ref(parsedHero.value.image);
</script>

<template>
    <div :style="{ background: 'url(' + imgUrl + ')', 'background-size': 'cover', 'background-repeat': 'no-repeat' }">
        <div
            class="h-screen bg-opacity-50 bg-black flex items-center justify-center"
            style="background: rgba(0, 0, 0, 0.5)"
        >
            <div class="mx-2 text-center">
                <h1 class="text-white font-bold text-4xl xs:text-5xl md:text-6xl">
                    <span>{{ parsedHero.title }}</span>
                </h1>
                <div class="inline-flex">
                    ...
                    <a href="#about"
                        ><button class="py-3 px-4 my-5 mx-2 bg-transparent border-[3px] hover:bg-orange-500 border-orange-500 rounded-md font-bold text-white shadow-md transition duration-500 md:text-lg capitalize">
                            {{ parsedHero.btnRight }}
                        </button></a
                    >
                </div>
