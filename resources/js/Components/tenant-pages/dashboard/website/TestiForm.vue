...
function editHandler() {
    form.post(route('website.testimony.update', {
        _method: 'put'
    }));
}
    ...
    <form action="#" @submit.prevent="editHandler">
        <div class="grid grid-cols-2 gap-10 pl-8 py-6">
            <div id="left">
                ...
                <InputGroup label="Testimony Title" labelFor="title">
                    <InputText inputId="title" v-model:inputValue="title" name="input_title"
                ...
                <InputGroup label="Brief Text" labelFor="text">
                    <InputTextArea inputId="text" v-model:inputValue="text" name="input_testi_text"
                ...
                <InputGroup label="Button text" labelFor="button">
                    <InputText inputId="button" v-model:inputValue="button" name="input_button_text"
            ...
            <div id="right" class="overflow-y-scroll max-h-[750px]">
                <div v-for="(card, index) in cards" class="pt-4 mr-4">
                    ...
                    <div class="px-4 py-2 mt-4 bg-primary-800 rounded-lg">
                        <InputGroup label="Testimony Title" lableFor="card-title">
                            <InputText inputId="card-title" v-model:inputValue="card.title"
                                input-placeholder="Your Card title Goes Here" />
                            <ValidationMessage v-if="getErrorMessage(index, 'title')"
                                :caption="getErrorMessage(index, 'title')" />
                        </InputGroup>
                        <InputGroup label="Testimony Text" lableFor="card-title">
                            <InputTextArea inputId="card-title" v-model:inputValue="card.text"
                                input-placeholder="Your Card title Goes Here" />
                            <ValidationMessage v-if="getErrorMessage(index, 'text')"
                                :caption="getErrorMessage(index, 'text')" />
                        </InputGroup>
                        <InputGroup label="Testimony Writer" lableFor="card-title">
                            <InputText inputId="card-title" v-model:inputValue="card.name"
                                input-placeholder="Your Card title Goes Here" />
                            <ValidationMessage v-if="getErrorMessage(index, 'name')"
                                :caption="getErrorMessage(index, 'name')" />
                        </InputGroup>
                        <InputGroup label="Testimony Rating" lableFor="card-title"
                            note="*the input is between 1 and 5*">
                            <InputText inputId="card-title" inputType="number" v-model:inputValue="card.stars"
                                input-placeholder="Your Card title Goes Here" />
                            <ValidationMessage v-if="getErrorMessage(index, 'stars')"
